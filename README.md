...
1. **Download**: Grab the latest `archive-manager-v1.0.0.zip` from the [Releases](https://github.com/OllyJohnston/OpenCode-Archive-Session-Manager/releases) page.
2. **Extract**: Unzip the folder to a permanent location (e.g., `C:\Plugins\ArchiveManager`).
3. **Register**: Add the folder path to your Opencode `opencode.jsonc` (on Windows) or via the TUI plugins menu:
   ```json
   {
     "plugins": [
